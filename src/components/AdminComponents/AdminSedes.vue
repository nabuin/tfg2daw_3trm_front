...
// llamar a la store de sedes con la data y mertodos a usar
...
// acceder a las sedes de sedesStore
    return sedesStore.sedes;
  }
  const query = searchQuery.value.toLowerCase();
  // filtro con sedesStore.sedes
...
const handleSedeSubmit = async (formData) => {
  ...
  try {
    ...
    console.error('Error al guardar sede:', error); // debug fallos
  }

  if (success) {
    closeSedeModal();
    // getall, obtener las sedes
